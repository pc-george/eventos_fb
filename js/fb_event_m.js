...
//Fecha: 30/10/2017
//Ultima modificación: 04/11/2017
	...
	for (i = 0; i < arregloDeEventos.data.length; i++) {
		...
		try {
			...
			if ( (country || city || namePlace || nameEvent || street || description) || ((lat) && (long)) ) {
				console.log("Ind. Cumple critero. " + rId);
				console.log(arregloDeEventos.data[i]);
				console.log(" ");
				contadorOk += 1;
				//var evento_temp = {id:rId, nameEvent:rNameEvent, namePlace:rNamePlace, street:rStreet, country:rCountry, city:rCity, description:rDescription, start_time:rStart_time, end_time:rEnd_time};
				evento_temp = {id:rId};
//				datos.push(evento_temp);

				var agregar = true;
				for (i = 0; i < datos.length; i++) {
					if (evento_temp.id === datos[i].id) {
						agregar = false;
						console.log("index[" + i + "]: " + datos[i].id + " == " + evento_temp.id); 
					}
				}
